...
async function fetchHeroes() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `We could not reach the heroes: ${response.status} ${response.statusText}`
      );
    }
...
async function fetchComics(heroId) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `We could not reach the comics: ${response.status} ${response.statusText}`
      );
    }
    const data = await response.json();
...
function displayItems(items, type) {
  ...
  const h2 = document.querySelector("#hero-area h2");
  if (items.length === 0) {
    const message = document.createElement("p");
    message.textContent = `No ${type.toLowerCase()} found.`;
